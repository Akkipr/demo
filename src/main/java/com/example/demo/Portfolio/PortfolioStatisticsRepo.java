...
public interface PortfolioStatisticsRepo extends JpaRepository<Portfolio, Long> {
    ...
    @Query(value =
                    "WITH merged AS ( " +
                    "    SELECT timestamp, symbol, close FROM public.stocks WHERE symbol = ?1 AND timestamp >= ?2 AND timestamp <= ?3 " +
                    "    UNION ALL " +
                    "    SELECT timestamp, symbol, close FROM public.newstocks WHERE symbol = ?1 AND timestamp >= ?2 AND timestamp <= ?3 " +
                    "), " +
                    "returns AS ( " +
